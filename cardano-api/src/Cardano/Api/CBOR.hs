...
pubKeyInfoFromCBOR bs =
  ...
  where
    ...
    decode = do
      tag <- CBOR.decodeWord8
      case tag of
        174  -> PubKeyInfoByron <$> networkFromCBOR <*> fromCBOR
        175  -> pure PubKeyInfoShelley
...
pubKeyInfoToCBOR pki =
  CBOR.serializeEncoding' $
    case pki of
      PubKeyInfoByron nw vk -> mconcat [ toCBOR (174 :: Word8), networkToCBOR nw, toCBOR vk ]
      PubKeyInfoShelley -> toCBOR (175 :: Word8)
